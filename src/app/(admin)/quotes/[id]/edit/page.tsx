...
export default function EditQuotePage() {
    ...
    React.useEffect(() => {
        const fetchQuote = async () => {
            ...
            const savedDraft = localStorage.getItem(`quote_draft_${quoteId}`);
            if (savedDraft) {
                try {
            ...
            // If no draft, fetch from DB
            const supabase = createClient();
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const { data: order, error } = await (supabase as any).from('orders')
                .select('*')
                .eq('id', quoteId)
                .single();
            
            if (order && !error) {
                if (order.customer_data) form.reset(order.customer_data);
                ...
                if (orderItems) {
                    ...
                    const mappedItems = orderItems.map((oi: any) => {
                        ...
                        let marginVal = 0;
                        if (cost > 0) marginVal = ((price - cost) / cost) * 100;
                        return {
                            id: prod.id,
                            name: prod.name || "Producto Removido",
                            sku: prod.sku || "N/A",
                            cost: cost,
                            price: price,
    ...
    React.useEffect(() => {
        const subscription = form.watch((value) => {
            const draft = { items, customer: value };
            if (quoteId) localStorage.setItem(`quote_draft_${quoteId}`, JSON.stringify(draft));
        });

        const currentForm = form.getValues();
        if ((items.length > 0 || currentForm.name) && quoteId) {
            const draft = { items, customer: currentForm };
            localStorage.setItem(`quote_draft_${quoteId}`, JSON.stringify(draft));
        }
    ...
    const clearDraft = () => {
        if (confirm("¿Estás seguro de limpiar los cambios de esta cotización?")) {
            localStorage.removeItem(`quote_draft_${quoteId}`);
    ...
    const onSubmit = async (customerData: z.infer<typeof customerSchema>) => {
        ...
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const { error: orderError } = await (supabase.from('orders') as any).update({
                total: total,
            ...
            // 3. Re-create Order Items
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            await (supabase.from('order_items') as any).delete().eq('order_id', quoteId);
            ...
            localStorage.removeItem(`quote_draft_${quoteId}`);
            router.push(`/quotes/${quoteId}`); // Go back to view page
    ...
    if (isLoadingData) {
        return <div className="p-12 flex justify-center text-muted-foreground">Cargando cotización...</div>;
    }
